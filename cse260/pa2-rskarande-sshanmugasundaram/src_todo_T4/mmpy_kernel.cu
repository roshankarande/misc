...
__global__ void matmul_2dtile_tilescale(int N, _FTYPE_ *C, _FTYPE_ *A, _FTYPE_ *B)
{
    ...
    int mpad = ceil(N / ((float)TW_Y * TS * dim_by)) * TW_Y * TS * dim_by;
    int npad = ceil(N / ((float)TW_X * TS * dim_bx)) * TW_X * TS * dim_bx;
    int kpad = ceil(N / ((float)TW_K)) * TW_K;

    _FTYPE_ _c_[TS][TS][TW_Y][TW_X] = {0.0f};

    for (int tbase = 0; tbase < kpad; tbase += TW_K)
    {
...
#pragma unroll
            ...
            {
                if ((row * TW_Y + twy + (mi * mpad / TS)) < N && (tbase + tx) < N)
                    Mdss(mi, twy, twx, ty, tx) = A(row * TW_Y + twy + (mi * mpad / TS), tbase + tx);
                else
...
#pragma unroll
            ...
            {
                if ((tbase + ty) < N && (col * TW_X + twx + (ni * npad / TS)) < N)
                    Ndss(ni, twy, twx, ty, tx) = B(tbase + ty, col * TW_X + twx + (ni * npad / TS));
                else
...
#pragma unroll
                for (int twx = 0; twx < TW_X; twx++)
                {
                    if ((row * TW_Y + twy + (mi * mpad / TS)) < N && (col * TW_X + twx + (ni * npad / TS)) < N)
                        C(row * TW_Y + twy + (mi * mpad / TS), col * TW_X + twx + (ni * npad / TS)) = _c_[mi][ni][twy][twx];
                }
            }
        }
    }
...
__global__ void matmul_tilescale(int N, _FTYPE_ *C, _FTYPE_ *A, _FTYPE_ *B)
{
    ...
    int mpad = ceil(N / ((float)TS)) * TS;
    int kpad = ceil(N / ((float)TW)) * TW;
...
#pragma unroll
        for (int mi = 0; mi < TS; mi++)
        {
            if (row + mi * mpad / TS < N && tbase + tx < N)
...
#pragma unroll
        for (int ni = 0; ni < TS; ni++)
        {
            if ((row + mi * mpad / TS) < N && (col + ni * npad / TS) < N)
